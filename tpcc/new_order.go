...
func buildStockSelectArgs(items []orderItem) []interface{} {
...
func stockUpdateArgs(item *orderItem) []interface{} {
...
func (w *Workloader) runNewOrder(ctx context.Context, thread int) error {
	...
	selectStockArgs := buildStockSelectArgs(items)
	...
	for i := 0; i < d.oOlCnt; i++ {
		...
		if _, err = s.newOrderStmts[newOrderUpdateStock].ExecContext(ctx, stockUpdateArgs(item)...); err != nil {
